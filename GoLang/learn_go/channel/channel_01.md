...
func main() {
	...
	var aworker = createWorker(0)
	var values []int

	// for 循环执行10s return
	tm := time.After(10 * time.Second)
	// 每隔time.Second时间往tick channel送一个数，我们收到这个数，也就是每隔1秒打印现在的len
	...
	for {
		...
		case n := <-c2:
			values = append(values, n)
		case activeWorker <- activeValue:   // 送到aworker worker去接受
			values = values[1:]
		// 两次select之间 如果间隔超过800 * time.Millisecond 就进入这个case
