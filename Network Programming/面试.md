...
|传输层| 段或报文 总stream 分段segment/message |传输层负责将上层数据分段并提供端到端的、可靠的或不可靠的传输。此外，传输层还要处理端到端的差错控制和流量控制问题| tcp udp | 网关 | 20Bytes|可靠|
|网络层| IP数据报   总datagram  分段fragment | 寻址/路由选择/连接的建立、保持和终止| IP协议族  地址解析协议ARP、逆地址解析协议RARP、因特网报文协议ICMP、IGMP | 路由器 | 20Bytes |IP层不可靠、无连接。功能：无连接数据报传输、数据报路由选择、差错控制。|
