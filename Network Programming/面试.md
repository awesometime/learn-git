...
||ARP reply|     网关路由器a端口 MAC|    A MAC|              网关路由器a端口-IP|     A-IP|
...
|接下来原路返回||||||
|省略 c学习路由器c端口mac 过程 ||||||							
|原路返回 主机c-路由器c端口 | ICMP answer | C MAC | 网关路由器c端口 MAC | C-IP |  A-IP |
|省略 路由器a端口学习a主机mac 过程||||||
|路由器a端口-主机a|ICMP answer | 网关路由器c端口 MAC | A MAC | C-IP | A-IP |
