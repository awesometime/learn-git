...
<style> table th:first-of-type(7) {     width: 20px; } </style>
<style> table th:first-of-type(3) {     width: 20px; } </style>
...
|网络层| IP数据报   总datagram  分段fragment | 寻址/路由选择/连接的建立、保持和终止| IP协议族 地址解析协议ARP、逆地址解析协议RARP、因特网报文协议ICMP、IGMP | 路由器 | 20Bytes |IP协议仅仅提供不可靠、无连接的传送服务。IP协议的主要功能有：无连接数据报传输、数据报路由选择和差错控制。|
