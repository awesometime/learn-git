...
- InnoDB: 主键的data放数据, 非主键的其他索引的data域存储主键的值, 所以回表

InnoDB 数据文件本身就是索引文件,表数据文件本身就是按B+Tree组织的一个索引结构，树的叶节点data域保存了完整的数据记录。

这个索引的key是数据表的主键，因此InnoDB表数据文件本身就是主索引。这被称为**聚集索引**。

而其余的索引都作为**非聚集索引**，**非聚集索引的data域存储相应记录主键的值**，需要回表。
