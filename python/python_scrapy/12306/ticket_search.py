...
class ticker_Spider(object):
    ...
    # todo 虽然验证失败不登录也能查票，但最好加上验证登录后再查票
    def search_Ticket(self):
        self.ls.main()
        # 'https://kyfw.12306.cn/otn/leftTicket/queryA?leftTicketDTO.train_date=2018-10-04&leftTicketDTO.from_station=CQW&leftTicketDTO.to_station=SHH&purpose_codes=ADULT'
        raw_from_station = input("请输入出发地>> ")
        raw_to_station = input("请输入目的地>> ")
        train_date = input("请输入出发日 (时间输入格式为 2018-10-02)  >> ")
    ...
    def print_TicketInfo(self):
        ...
        pt.add_column('出发日期', from_datetime)
