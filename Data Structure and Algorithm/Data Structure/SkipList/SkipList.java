package com.ctcc.cnp.app.controller.northbound;
...
public class SkipList<E extends Comparable<? super E>> implements Iterable<E> {
    ...
    private Node find(E e, Node current, int level) {
        // 如果当前level没有就往下边level找
        // 当前level没有,上边level是不可能有的
        while (level >= 0) {
            current = findNext(e, current, level);
            level--;
        }
    ...
    private class Node {
        ...
        // 保存了每一层上右边一个节点的节点信息，可能为null
        // 最下边是level 0
        // 指向节点E的下一个(右边一个)节点
        // 4层 forwards=[node(5), node(6), null, null]
        // 代表当前节点E在level=0层的右边是node(5),在level=1层的右边是node(6)
        // forwards 指的是当前节点在每一层指向的下一个节点
        List<Node> forwards;
